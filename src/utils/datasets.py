...
class DataManager(object):
    ...
    def __init__(self, dataset_name='imdb',
        ...
        elif self.dataset_name == 'kor_multi_modal':
#             self.dataset_path='../datasets/kor_multi_modal/prep_col_all_mdf.csv'
            self.dataset_path='../datasets/kor_multi_modal/prep_col_all_mdf_frontal_face.csv'
        else:
            raise Exception(
    ...
    def get_data(self):
        if self.dataset_name == 'imdb':
            ...
            ground_truth_data = self._load_kor_multi_modal()
    ...
    def _load_kor_multi_modal(self):
        data = pd.read_csv(self.dataset_path).sample(frac=1).reset_index(drop=True)
        pixels = data['pixels'].tolist()
        width, height = 48, 48
        faces = []
...
def get_labels(dataset_name):
    if dataset_name == 'fer2013' or dataset_name == 'kor_multi_modal':
